...
namespace reservation_service.Repository
{
    public class ReservationRequestRepository : IReservationRequestRepository
    {
        ...
        public async Task<List<ReservationRequest>> GetAllByGuestUsernameAsync(string username) =>
            await _reservationRequestsCollection.Find(x => x.GuestUsername.Equals(username)).ToListAsync();
